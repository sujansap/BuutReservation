...
{
    public class ReservationControllerTest(ApiWebApplicationFactory fixture) : IntegrationTest(fixture)
    {
        [Fact]
        public async Task GET_CurrentUser_UpcomingReservations_WithNoParameters_FirstPage_ExpectOk_5OrLessReservations()
        {
            var response = await _client.GetAsync($"Reservation/me");
        ...
        {
            var response = await _client.GetAsync($"Reservation/me?getPast=true");
            response.StatusCode.ShouldBe(HttpStatusCode.OK);
        ...
        {
            var response = await _client.GetAsync($"Reservation/me?pageSize={pageSize}");
            response.StatusCode.ShouldBe(HttpStatusCode.OK);
        ...
        {
            var response = await _client.GetAsync($"Reservation/me?cursor={cursor}");
            response.StatusCode.ShouldBe(HttpStatusCode.BadRequest);
        }
        ...
        [Fact]
        public async Task GET_CurrentUser_Reservations_NextPage_WithValidCursor_InavlidIsNextPage_ExpectBadRequest()
        {
            var response = await _client.GetAsync($"Reservation/me?cursor=17");
            response.StatusCode.ShouldBe(HttpStatusCode.BadRequest);
        }
    }
}
